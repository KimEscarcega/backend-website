...
create table feedback(fID int unsigned not null, uID int unsigned, fdate date, text varchar(150), primary key(fID), foreign key(uID) references user(uID) on update cascade);
create table vehicle(vID varchar(10) not null, uID int unsigned, make varchar(60), model varchar(60), color varchar(60), primary key(vID), foreign key(uID) references user(uID) on update cascade);
