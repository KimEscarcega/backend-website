...
create table report(rID int unsigned not null auto_increment, uID int unsigned, rdate date, rtime time, rimage BLOB, primary key(rID), foreign key(uID) references user(uID) on update cascade);
